  ...
  <div class="page-editor">
    ...
    <header class="editor-toolbar">
      ...
      <div class="toolbar-group">
        ...
        <n-button text title="Ajouter un emoji" @click="showEmojiPicker = true">
          😀 Emoji
        </n-button>
        <n-button text title="Créer une forme" @click="showShapePicker = true">
          🟥 Forme
        </n-button>
        <n-button
          text
    ...
    <div class="editor-content">
        ...
        <div class="canvas-container">
          <div class="canvas">
            ...
            <CanvasElement
              ...
              :element="element"
              :isSelected="
                pageElementsStore.selectedElementIds.length > 0 &&
                pageElementsStore.selectedElementIds[0] === element.id
              "
              :isMultiSelected="
                pageElementsStore.isSelected(element.id) &&
                pageElementsStore.selectedElementIds.length > 1
              "
              :canvasWidth="2100"
              :canvasHeight="2970"
      ...
      <aside v-if="showRightSidebar" class="editor-sidebar-right-new">
        ...
        <PropertiesPanel
          ...
          @duplicate="
            pageElementsStore.selectedElementIds.length > 0 &&
              handleElementDuplicated(pageElementsStore.selectedElementIds[0])
          "
          @delete="
            pageElementsStore.selectedElementIds.length > 0
          ...
          @restore="
            pageElementsStore.selectedElementIds.length > 0 &&
              handleElementRestored(pageElementsStore.selectedElementIds[0])
          "
          @transform="
            pageElementsStore.selectedElementIds.length > 0 &&
              handleImageTransformRequest(
                pageElementsStore.selectedElementIds[0]
              )
          "
...
function handleElementSelected(elementId: string): void {
  ...
  const element = pagesStore.elements[currentPageId]?.find(
...
function handleElementModified(elementId: string, changes: any): void {
  try {
    ...
    if (editorStore.canvas) {
      const currentState = fabricService.serializeCanvasElements(
        editorStore.canvas
      );
...
async function handleShapeCreated(element: IPageElement): Promise<void> {
  try {
    ...
    const errorMsg =
      error instanceof Error
        ? error.message
        : "Erreur lors de l'ajout de la forme";
    message.error(errorMsg);
 ...
 */
function handleCanvasElementSelected(
  elementId: string,
 ...
 */
async function handleElementResized(
  id: string,
  width: number,
  height: number
): Promise<void> {
 ...
 */
async function handleElementRotated(
  id: string,
  rotation: number
): Promise<void> {
  try {
 ...
 */
async function handlePropertiesUpdated(
  id: string,
  data: IPageElementUpdateRequest
): Promise<void> {
  try {
...
async function handleStickerDropped(
  ...
  try {
    ...
    const errorMsg =
      error instanceof Error
        ? error.message
        : "Erreur lors de l'ajout du sticker";
    message.error(errorMsg);
 ...
 */
async function handleImageTransformApplied(
  ...
  try {
    ...
    const response: IImageTransformationResponse =
      await mediaService.transformImage(
...
function handleTextUpdatedFromSidebar(
  ...
  try {
    ...
    console.log(
      `Text element updated from sidebar: ${editorStore.selectedElement.id}`
    );
  } catch (error) {
...
async function handleAutoSaveCallback(): Promise<void> {
  ...
  try {
    ...
    const payload = fabricService.serializeCanvasForBackend(
      editorStore.canvas,
      pageId.value
    );

    // Call batch save API
    const result = await pageService.saveElements(
      pageId.value,
    ...
    editorStore.setAutoSaveStatus("saved");
    console.log(
      `Auto-save successful: created=${result.created}, updated=${result.updated}`
    );
  } catch (error) {
...
function handleKeyDown(event: KeyboardEvent): void {
  ...
  if ((event.ctrlKey || event.metaKey) && event.key === "a") {
    ...
    console.log(
      `All ${pageElementsStore.getSelectedCount()} elements selected (Ctrl+A)`
    );
  }
...
onMounted(async () => {
  ...
  window.addEventListener("offline", () => {
    ...
    message.warning(
      "Mode hors ligne - Vos modifications seront sauvegardées localement"
    );
  });
  ...
  try {
    ...
    if (currentPageData) {
      ...
      pageOrientation.value =
        (currentPageData as any).orientation || "portrait";
    } else {
    ...
    try {
      ...
      console.log(
        `Phase 4 elements loaded: ${pageElementsStore.getElementCount}`
      );
    } catch (err) {
    ...
    console.log(
      `PageEditor loaded for notebook ${notebookId.value}, page ${pageId.value}`
    );
  } catch (error) {
